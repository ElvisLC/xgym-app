        ...
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          <div ref={pillarsRef}>
            ...
            <div className="hidden md:block relative mb-20">
              ...
              <div className="absolute top-[19px] left-0 right-0 h-0.5">
                <motion.div
              ...
              <div className="grid grid-cols-5 gap-4 relative z-10">
                {PILLARS.map((p, index) => {
                  ...
                  const isCurrent = activePillar === index

                  return (
                    <div key={p.num} className="flex flex-col">
                      <div className="flex justify-center mb-6">
                        <motion.div
                          className={`w-10 h-10 flex items-center justify-center border-2 font-mono text-xs font-bold transition-colors duration-300 ${
                            isActive
                              ? 'bg-[var(--canvas)] border-[var(--accent)] text-[var(--accent)]'
                              : 'bg-[var(--surface)] border-white/10 text-[var(--subtle)]'
                          }`}
                      ...
                      <div className={`flex-1 border p-5 transition-all duration-300 ${
                        isActive ? 'border-[var(--accent)]/40 bg-[var(--surface)]' : 'border-white/10 bg-[var(--surface)]'
                      }`}>
                        <h3 className={`font-semibold text-sm mb-2 transition-colors duration-300 ${
                          isActive ? 'text-white' : 'text-[var(--subtle)]'
                        }`}>{p.title}</h3>
                ...
                {PILLARS.map((p, index) => {
                  ...
                  return (
                    <div key={p.num} className="relative">
                      <motion.div
                        className={`absolute left-[-33px] top-3 w-8 h-8 flex items-center justify-center border-2 font-mono text-[10px] font-bold z-10 transition-colors duration-300 ${
                          isActive
                            ? 'bg-[var(--canvas)] border-[var(--accent)] text-[var(--accent)]'
                            : 'bg-[var(--surface)] border-white/10 text-[var(--subtle)]'
                        }`}
